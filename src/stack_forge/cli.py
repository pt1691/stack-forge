...
def init(
    ...
    if config_path.exists():
        if not typer.confirm(f"⚠️ {config_path} already exists. Overwrite?"):
            raise typer.Exit(0)
...
def add_resource(
    ...
    console.print(f"✅ Resource [cyan]{resource_name}[/cyan] ({resource_type.value}) added to stack [green]{stack_name}[/green]")
...
def validate(
    ...
    try:
        ...
        duplicate_stacks = set(k for k in stack_keys if stack_keys.count(k) > 1)
        if duplicate_stacks:
